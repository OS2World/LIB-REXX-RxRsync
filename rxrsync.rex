...
"client/server differencing" protocol. They require RxRsync.DLL 
(version 1.01).
...
There are three procedures:
   ...
   rsync_synopsis: creates a synopsis of an old version.
       ...
       Note that rsync_synopsis has a few user changeable parameters
   ...
   rsync_ungdiff: uses a "gdiff-formatted" difference file, and an
                      the old version, to build a copy of the new version

      status=rsync_ungdiff(oldver_file,diff_file,newver_file,amd4,quiet)

      quiet and amd4 are optional parameters
